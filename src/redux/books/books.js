...
const app_id = 'dGG4fOJN0KtR2qngOQAi';
export const fetchBooks = createAsyncThunk('books/fetchBooks', async () => {
  const response = await axios.get(
    `https://us-central1-bookstore-api-e63c8.cloudfunctions.net/bookstoreApi/apps/${app_id}/books`,
  );
  const books = Object.entries(response.data).flatMap(([item_id, books]) => books.map((book) => ({ ...book, item_id })));
...
const bookSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(removeBook.fulfilled, (state, action) => state.filter((book) => book.item_id !== action.payload));
  },
});
